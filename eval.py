...
def main(pretrained, data_files, model_params, training_params, device):
    """ forward pass test data to trained model  """
    ...
    # Data loading code 
    test_data = SpoofDatsetSystemID(data_files['test_scp'], data_files['test_utt2index'], binary_class=True)
    ...
    print("===> forward pass for test set")
    score_file_pth = os.path.join(data_files['scoring_dir'], str(pretrained_id) + '-epoch%s-test_scores.json' %(epoch_id))
    print("===> test scoring file saved at: '{}'".format(score_file_pth))
    prediction.prediction(test_loader, model, device, score_file_pth,data_files['test_utt2index'])


if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--data-feats', action='store', type=str, default='cqt')
